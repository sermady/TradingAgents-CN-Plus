...
DEFAULT_CONFIG = {
    ...
    # ========== 实时行情配置 ==========
    # 实时行情功能开关（环境变量可覆盖）
    "realtime_data_enabled": os.getenv("REALTIME_QUOTE_ENABLED", "true").lower()
    == "true",
    # 实时行情详细配置
    "realtime_data": {
        "enabled": True,  # 是否启用实时行情
        "auto_detect_trading_hours": True,  # 自动检测交易时段
        "preferred_source": "akshare",  # 首选数据源：akshare（免费、秒级）
        "fallback_source": "tushare",  # 备选数据源：tushare
        "fallback_to_close": True,  # 非交易时段回退到收盘价
        "cache_ttl_seconds": 60,  # 实时数据缓存时间（秒）
    },
    # ========== 历史行情配置 ==========
    # 历史行情数据源优先级（从.env读取，不再查询数据库）
    "historical_data_priority": os.getenv(
        "HISTORICAL_DATA_SOURCE_PRIORITY", "tushare,akshare,baostock"
    ),
    # MongoDB缓存策略
    "skip_mongodb_cache_on_query": os.getenv(
        "SKIP_MONGODB_CACHE_ON_QUERY", "true"
    ).lower()
    == "true",
    "save_to_mongodb_after_query": os.getenv(
        "SAVE_TO_MONGODB_AFTER_QUERY", "true"
    ).lower()
    == "true",
    # 默认分析日期配置
