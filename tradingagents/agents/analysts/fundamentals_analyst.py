    ...
    def fundamentals_analyst_node(state):
        ...
        if GoogleToolCallHandler.is_google_model(fresh_llm):
            ...
            if current_tool_calls > 0:
                ...
                # 注意：仅当有工具结果时，分析内容才被认为有效
                # 避免LLM在没有工具数据的情况下直接编造分析
                ...
                if hasattr(result, 'content') and result.content:
                    ...
                    logger.info(f"[内容检查] LLM未返回内容或内容为空")
                ...
                # 关键修复：必须有工具结果才能跳过强制调用
                # 仅有分析内容（无工具数据）时仍需强制调用工具，避免LLM编造数据
                if has_tool_result:
                ...
                elif has_analysis_content:
                    # 有分析内容但没有工具结果 - 警告并强制调用工具
                    logger.warning(f"[警告] LLM返回了分析内容但未调用工具，可能是编造数据，强制调用工具获取真实数据")
