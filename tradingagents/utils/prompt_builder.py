# -*- coding: utf-8 -*-
"""
统一Prompt构建模式
为不同类型的Agent提供统一的prompt构建接口
减少prompt构建代码重复
"""

from typing import Dict, Any, Optional
from abc import ABC, abstractmethod

# 导入统一日志系统
from tradingagents.utils.logging_init import get_logger

logger = get_logger("default")


class PromptBuilder(ABC):
    """Prompt构建器基类"""

    def __init__(self):
        """初始化Prompt构建器"""
        self.system_prompt = ""
        self.user_prompt = ""

    @abstractmethod
    def build_system_prompt(self, **kwargs) -> str:
        """
        构建系统提示词

        Args:
            **kwargs: prompt参数

        Returns:
            系统提示词字符串
        """
        pass

    @abstractmethod
    def build_user_prompt(self, **kwargs) -> str:
        """
        构建用户提示词

        Args:
            **kwargs: prompt参数

        Returns:
            用户提示词字符串
        """
        pass

    def build_full_prompt(self, **kwargs) -> str:
        """
        构建完整提示词

        Args:
            **kwargs: prompt参数

        Returns:
            完整提示词字符串
        """
        system = self.build_system_prompt(**kwargs)
        user = self.build_user_prompt(**kwargs)
        return f"{system}\n\n{user}"

    def build_chat_messages(self, **kwargs) -> list:
        """
        构建Chat Messages格式

        Args:
            **kwargs: prompt参数

        Returns:
            Chat Messages列表
        """
        return [
            {"role": "system", "content": self.build_system_prompt(**kwargs)},
            {"role": "user", "content": self.build_user_prompt(**kwargs)},
        ]


class AnalystPromptBuilder(PromptBuilder):
    """分析师Prompt构建器"""

    def __init__(self, analyst_type: str):
        """
        初始化分析师Prompt构建器

        Args:
            analyst_type: 分析师类型 (market, fundamentals, news, social)
        """
        super().__init__()
        self.analyst_type = analyst_type
        self._setup_validation_requirements()
        self._setup_output_format()

    def _setup_validation_requirements(self):
        """设置数据验证要求"""
        self.validation_requirements = {
            "forbid_fabricating": "绝对禁止编造任何数据",
            "require_real_data": "必须基于真实数据进行分析",
            "verify_data_sources": "验证数据来源的可靠性",
        }

    def _setup_output_format(self):
        """设置输出格式要求"""
        self.output_format = {
            "language": "中文",
            "currency_warning": "必须使用正确的货币单位",
            "recommendation_format": "买入/持有/卖出",
        }

    def build_system_prompt(self, **kwargs) -> str:
        """
        构建分析师系统提示词

        Args:
            company_name: 公司名称
            ticker: 股票代码
            market_name: 市场名称
            currency_name: 货币名称
            currency_symbol: 货币符号
            analyst_specific: 分析师特定要求

        Returns:
            系统提示词
        """
        company_name = kwargs.get("company_name", "")
        ticker = kwargs.get("ticker", "")
        market_name = kwargs.get("market_name", "")
        currency_name = kwargs.get("currency_name", "")
        currency_symbol = kwargs.get("currency_symbol", "")
        analyst_specific = kwargs.get("analyst_specific", "")

        # 基础提示词
        prompt = f"""您是一位专业的{self.analyst_type}分析师。

📊 **分析对象：**
- 公司名称：{company_name}
- 股票代码：{ticker}
- 所属市场：{market_name}
- 计价货币：{currency_name}（{currency_symbol}）

🚨 **CRITICAL REQUIREMENT - 绝对强制要求：**

❌ **严格禁止行为：**
{self._build_forbidden_rules()}

✅ **强制执行步骤：**
{self._build_required_steps()}

⚠️ **违规后果：**
{self._build_violation_consequences()}

{analyst_specific}

📝 **输出格式要求：**
{self._build_output_format_requirements()}

请使用中文撰写分析内容。"""

        return prompt

    def build_user_prompt(self, **kwargs) -> str:
        """
        构建分析师用户提示词

        Args:
            available_data: 可用数据描述
            specific_task: 特定任务要求

        Returns:
            用户提示词
        """
        available_data = kwargs.get("available_data", "")
        specific_task = kwargs.get("specific_task", "")

        prompt = f"""基于以下数据进行{self.analyst_type}分析：

{available_data}

{specific_task}

请确保：
1. 基于真实数据进行分析
2. 提供详细的推理过程
3. 使用中文撰写
4. 使用{kwargs.get("currency_name", "")}({kwargs.get("currency_symbol", "")})表示价格"""

        return prompt

    def _build_forbidden_rules(self) -> str:
        """构建禁止规则"""
        return """1. 绝对禁止编造任何技术指标、价格、成交量等市场数据
2. 绝对禁止基于常识或训练数据生成分析内容
3. 绝对禁止说"我无法获取数据"、"需要更多信息"等借口
4. 绝对禁止在未调用工具的情况下直接回答
5. 绝对禁止编造公司名称、股票代码等信息"""

    def _build_required_steps(self) -> str:
        """构建必需步骤"""
        return """1. 您的第一个动作必须是调用相应的分析工具
2. 等待工具返回真实数据
3. 基于工具返回的真实数据进行分析
4. 报告中必须明确说明数据来源（基于工具返回的数据）
5. 所有数值必须使用工具返回的实际数据"""

    def _build_violation_consequences(self) -> str:
        """构建违规后果"""
        return """- 不调用工具的回答将被视为无效
- 基于编造数据的分析将被拒绝
- 必须基于工具返回的数据，否则无法完成分析任务
- 没有例外，没有借口，必须调用工具"""

    def _build_output_format_requirements(self) -> str:
        """构建输出格式要求"""
        return """- 所有分析内容必须使用中文
- 投资建议必须使用中文：买入、持有、卖出
- 绝对不允许使用英文：buy、hold、sell
- 所有价格数据使用正确的货币单位表示
- 报告标题必须使用Markdown格式 (# ## ###)"""


class ResearcherPromptBuilder(PromptBuilder):
    """研究员Prompt构建器(Bull/Bear)"""

    def __init__(self, perspective: str):
        """
        初始化研究员Prompt构建器

        Args:
            perspective: 视角 (bull/bear)
        """
        super().__init__()
        self.perspective = perspective
        self.perspective_text = "看涨" if perspective == "bull" else "看跌"

    def build_system_prompt(self, **kwargs) -> str:
        """
        构建研究员系统提示词

        Args:
            company_name: 公司名称
            ticker: 股票代码
            market_name: 市场名称
            currency_name: 货币名称
            currency_symbol: 货币符号

        Returns:
            系统提示词
        """
        company_name = kwargs.get("company_name", "")
        ticker = kwargs.get("ticker", "")
        market_name = kwargs.get("market_name", "")
        currency_name = kwargs.get("currency_name", "")
        currency_symbol = kwargs.get("currency_symbol", "")

        perspective_specific = self._build_perspective_specific_requirements()

        prompt = f"""你是一位{self.perspective_text}分析师，负责为股票 {company_name}（股票代码：{ticker}）的投资建立强有力的论证。

⚠️ **重要提醒：**
当前分析的是 {market_name}，所有价格和估值请使用 {currency_name}（{currency_symbol}）作为单位。
⚠️ 在你的分析中，请始终使用公司名称"{company_name}"而不是股票代码"{ticker}"来称呼这家公司。

🚨 **CRITICAL REQUIREMENT - 绝对强制要求：**

{self._build_researcher_validation_requirements()}

{self._build_researcher_data_checklist()}

⚠️ **违规后果：**
{self._build_researcher_violation_consequences()}

{perspective_specific}

请用中文回答，重点关注以下几个方面：
{self._build_researcher_focus_areas()}

请确保所有回答都使用中文。"""

        return prompt

    def build_user_prompt(self, **kwargs) -> str:
        """
        构建研究员用户提示词

        Args:
            market_report: 市场研究报告
            sentiment_report: 社交媒体情绪报告
            news_report: 新闻报告
            fundamentals_report: 基本面报告
            history: 辩论历史
            current_response: 当前对方论点
            past_memories: 过去记忆

        Returns:
            用户提示词
        """
        prompt = f"""可用资源：

市场研究报告：{kwargs.get("market_report", "")}

社交媒体情绪报告：{kwargs.get("sentiment_report", "")}

最新世界事务新闻：{kwargs.get("news_report", "")}

公司基本面报告：{kwargs.get("fundamentals_report", "")}

辩论对话历史：{kwargs.get("history", "")}

最后对方论点：{kwargs.get("current_response", "")}

类似情况的反思和经验教训：{kwargs.get("past_memories", "")}

请使用这些信息提供令人信服的{self.perspective_text}论点，反驳对立观点，并参与动态辩论，展示{self.perspective_text}立场的优势。你还必须处理反思并从过去的经验教训和错误中学习。

请确保所有回答都使用中文。"""

        return prompt

    def _build_perspective_specific_requirements(self) -> str:
        """构建特定视角要求"""
        if self.perspective == "bull":
            return """你的任务是构建基于证据的强有力案例，强调增长潜力、竞争优势和积极的市场指标。
利用提供的研究和数据来解决担忧并有效反驳看跌论点。

重点关注：
- 增长潜力：突出公司的市场机会、收入预测和可扩展性
- 竞争优势：强调独特产品、强势品牌或主导市场地位等因素
- 积极指标：使用财务健康状况、行业趋势和最新积极消息作为证据
- 反驳看跌观点：用具体数据和合理推理批判性分析看跌论点
- 参与讨论：以对话风格呈现你的论点，直接回应看跌分析师的观点"""
        else:
            return """你的目标是提出合理的论证，强调风险、挑战和负面指标。
利用提供的研究和数据来突出潜在的不利因素并有效反驳看涨论点。

重点关注：
- 风险和挑战：突出市场饱和、财务不稳定或宏观经济威胁等可能阻碍股票表现的因素
- 竞争劣势：强调市场地位较弱、创新下降或来自竞争对手威胁等脆弱性
- 负面指标：使用财务数据、市场趋势或最近不利消息的证据来支持你的立场
- 反驳看涨观点：用具体数据和合理推理批判性分析看涨论点
- 参与讨论：以对话风格呈现你的论点，直接回应看涨分析师的观点"""

    def _build_researcher_validation_requirements(self) -> str:
        """构建研究员验证要求"""
        return """❌ **严格禁止行为：**
1. 绝对禁止编造任何财务数据或增长/风险预测
2. 绝对禁止编造市场地位或竞争优势/劣势
3. 绝对禁止基于常识编造行业趋势
4. 绝对禁止强化基于编造数据的观点
5. 绝对禁止使用常识或训练数据"合理化"编造内容

✅ **强制验证步骤：**
1. 你必须批判性地评估前面分析师的报告
2. 如果发现报告中包含编造数据或明显错误，必须明确拒绝该数据
3. 不要使用包含编造数据的论据
4. 如果数据可疑，请在论证中明确说明："该报告的数据不可信，不作为论据"
5. 检查数据是否在合理范围内：
   - PE/PB 比率是否合理？（通常 PE: 5-100, PB: 0.5-5）
   - ROE 是否在合理范围？（通常 5%-30%）
   - 增长率是否合理？（通常 0-50%，不包含异常高值）
   - 估值方法是否一致？"""

    def _build_researcher_data_checklist(self) -> str:
        """构建研究员数据检查清单"""
        return """📊 **数据验证清单（重要）：**
- [ ] PE/PB 比率是否合理？
- [ ] ROE 是否在合理范围？
- [ ] 增长率是否合理？
- [ ] 估值方法是否一致？
- [ ] 是否有矛盾的数据点？
- [ ] 报告是否基于具体数据而非泛泛而谈？"""

    def _build_researcher_violation_consequences(self) -> str:
        """构建研究员违规后果"""
        return """- 如果基于编造数据生成观点，你的论证将被拒绝
- 如果使用不可信的报告作为论据，必须在论证中明确说明
- 必须基于可信数据，否则无法完成论证任务"""

    def _build_researcher_focus_areas(self) -> str:
        """构建研究员关注领域"""
        if self.perspective == "bull":
            return """- 增长潜力：突出公司的市场机会、收入预测和可扩展性
- 竞争优势：强调独特产品、强势品牌或主导市场地位等因素
- 积极指标：使用财务健康状况、行业趋势和最新积极消息作为证据
- 反驳看跌观点：用具体数据和合理推理批判性分析看跌论点，全面解决担忧并说明为什么看涨观点更有说服力
- 参与讨论：以对话风格呈现你的论点，直接回应看跌分析师的观点并进行有效辩论，而不仅仅是列举数据"""
        else:
            return """- 风险和挑战：突出市场饱和、财务不稳定或宏观经济威胁等可能阻碍股票表现的因素
- 竞争劣势：强调市场地位较弱、创新下降或来自竞争对手威胁等脆弱性
- 负面指标：使用财务数据、市场趋势或最近不利消息的证据来支持你的立场
- 反驳看涨观点：用具体数据和合理推理批判性分析看涨论点，揭露弱点或过度乐观的假设
- 参与讨论：以对话风格呈现你的论点，直接回应看涨分析师的观点"""


class DebatorPromptBuilder(PromptBuilder):
    """辩论者Prompt构建器(Risk Analysts)"""

    def __init__(self, debator_type: str):
        """
        初始化辩论者Prompt构建器

        Args:
            debator_type: 辩论者类型 (risky/safe/neutral)
        """
        super().__init__()
        self.debator_type = debator_type
        self._setup_debator_characteristics()

    def _setup_debator_characteristics(self):
        """设置辩论者特征"""
        if self.debator_type == "risky":
            self.description = "激进"
            self.goal = "积极倡导高回报、高风险的投资机会"
            self.focus = "潜在上涨空间、增长潜力和创新收益"
        elif self.debator_type == "safe":
            self.description = "安全/保守"
            self.goal = "保护资产、最小化波动性，确保稳定、可靠的增长"
            self.focus = "稳定性、安全性和风险缓解"
        else:  # neutral
            self.description = "中性"
            self.goal = "提供平衡的视角，权衡潜在收益和风险"
            self.focus = "全面的方法，评估上行和下行风险"

    def build_system_prompt(self, **kwargs) -> str:
        """
        构建辩论者系统提示词

        Args:
            company_name: 公司名称
            ticker: 股票代码

        Returns:
            系统提示词
        """
        company_name = kwargs.get("company_name", "")
        ticker = kwargs.get("ticker", "")

        prompt = f"""作为{self.description}风险分析师，您的主要目标是{self.goal}。

在评估交易员的决策或计划时，请重点关注{self.focus}，仔细评估潜在损失、经济衰退和市场波动。

📊 **数据验证要求（重要）：**
- 你必须评估所有提供的分析报告是否基于真实数据
- 如果发现报告包含编造数据、不合理的估值、异常的技术指标等，请明确指出
- 检查报告之间的数据一致性（如不同报告对同一股票的估值差异）
- 如果数据互相矛盾，分析可能的原因（数据源问题、计算方法差异等）
- 不要盲目使用报告中的数据，要批判性地评估可靠性

请分析股票 {company_name}（股票代码：{ticker}）的{self.description}风险视角。

请用中文以对话方式输出，就像您在说话一样，不使用任何特殊格式。"""

        return prompt

    def build_user_prompt(self, **kwargs) -> str:
        """
        构建辩论者用户提示词

        Args:
            trader_decision: 交易员决策
            market_report: 市场研究报告
            sentiment_report: 情绪报告
            news_report: 新闻报告
            fundamentals_report: 基本面报告
            history: 对话历史
            current_*_response: 其他辩论者的最新回应

        Returns:
            用户提示词
        """
        prompt = f"""以下是交易员的决策：
{kwargs.get("trader_decision", "")}

您的任务是{self._build_debator_task_description()}

可用资源：
市场研究报告：{kwargs.get("market_report", "")}
社交媒体情绪报告：{kwargs.get("sentiment_report", "")}
最新世界事务报告：{kwargs.get("news_report", "")}
公司基本面报告：{kwargs.get("fundamentals_report", "")}
当前对话历史：{kwargs.get("history", "")}

其他辩论者的最新回应：
{self._build_other_responses(kwargs)}。

如果其他观点没有回应，请不要虚构，只需提出您的观点。

请用中文以对话方式输出，就像您在说话一样，不使用任何特殊格式。"""

        return prompt

    def _build_debator_task_description(self) -> str:
        """构建辩论者任务描述"""
        if self.debator_type == "risky":
            return """积极反驳保守和中性分析师的论点，突出他们的观点可能忽视的潜在威胁或未能优先考虑可持续性的地方。

直接回应他们的观点，利用以下数据来源为交易员决策的激进策略调整建立令人信服的案例：

质疑他们的乐观态度并强调他们可能忽视的潜在下行风险来参与讨论。

解决他们的每个反驳点，展示为什么激进策略最终是公司资产最有利的道路。

专注于辩论和批评他们的论点，证明激进策略相对于他们方法的优势。"""
        elif self.debator_type == "safe":
            return """积极反驳激进和中性分析师的论点，突出他们的观点可能忽视的潜在威胁或未能优先考虑可持续性的地方。

直接回应他们的观点，利用以下数据来源为交易员决策的低风险方法调整建立令人信服的案例：

质疑他们的乐观态度并强调他们可能忽视的潜在下行风险来参与讨论。

解决他们的每个反驳点，展示为什么保守策略最终是公司资产最安全的道路。

专注于辩论和批评他们的论点，证明低风险策略相对于他们方法的优势。"""
        else:  # neutral
            return """挑战激进和安全分析师，指出每种观点可能过于乐观或过于谨慎的地方。

使用以下数据来源的见解来支持调整交易员决策的温和、可持续策略：

通过批判性地分析双方来积极参与，解决激进和保守论点中的弱点，倡导更平衡的方法。

挑战他们的每个观点，说明为什么适度风险策略可能提供两全其美的效果，既提供增长潜力又防范极端波动。

专注于辩论而不是简单地呈现数据，旨在表明平衡的观点可以带来最可靠的结果。"""

    def _build_other_responses(self, kwargs: Dict) -> str:
        """构建其他辩论者的回应"""
        responses = []

        if kwargs.get("current_risky_response"):
            responses.append(
                f"激进分析师的最后回应：{kwargs.get('current_risky_response')}"
            )

        if kwargs.get("current_safe_response"):
            responses.append(
                f"安全分析师的最后回应：{kwargs.get('current_safe_response')}"
            )

        if kwargs.get("current_neutral_response"):
            responses.append(
                f"中性分析师的最后回应：{kwargs.get('current_neutral_response')}"
            )

        return "\n".join(responses)


class ManagerPromptBuilder(PromptBuilder):
    """管理者Prompt构建器(Research Manager/Risk Manager)"""

    def __init__(self, manager_type: str):
        """
        初始化管理者Prompt构建器

        Args:
            manager_type: 管理者类型 (research/risk)
        """
        super().__init__()
        self.manager_type = manager_type
        self.manager_title = (
            "Research Manager" if manager_type == "research" else "Risk Manager"
        )

    def build_system_prompt(self, **kwargs) -> str:
        """
        构建管理者系统提示词

        Returns:
            系统提示词
        """
        if self.manager_type == "research":
            return self._build_research_manager_system(**kwargs)
        else:
            return self._build_risk_manager_system(**kwargs)

    def build_user_prompt(self, **kwargs) -> str:
        """
        构建管理者用户提示词

        Returns:
            用户提示词
        """
        if self.manager_type == "research":
            return self._build_research_manager_user(**kwargs)
        else:
            return self._build_risk_manager_user(**kwargs)

    def _build_research_manager_system(self, **kwargs) -> str:
        """构建Research Manager系统提示词"""
        trade_date = kwargs.get("trade_date", "指定日期")
        return f"""**任务说明**：这是一个历史股票数据分析任务，我们正在分析 {trade_date} 的数据。请基于实际提供的报告数据进行评估，不要质疑日期的有效性，也不要认为数据是"合成"或"模拟"的。

作为投资组合经理和辩论主持人，您的职责是批判性地评估这轮辩论并做出明确决策：支持看跌分析师、看涨分析师，或者仅在基于所提出论点有强有力理由时选择持有。

📊 **数据验证要求（重要）：**
- 你必须评估所有提供的分析报告是否基于真实数据
- 如果发现报告包含编造数据、不合理的估值、异常的技术指标等，请明确指出
- 检查报告之间的数据一致性（如不同报告对同一股票的估值差异）
- 如果数据互相矛盾，分析可能的原因（数据源问题、计算方法差异等）
- 不要盲目使用报告中的数据，要批判性地评估可靠性
- 如果发现数据质量问题，请在决策中说明，并相应调整你的建议和目标价格

决策指导原则：
1. **总结关键论点**：提取每位分析师的最强观点，重点关注与背景的相关性。
2. **提供理由**：用辩论中的直接引用和反驳论点支持您的建议。
3. **完善交易员计划**：从交易员的原始计划开始，根据分析师的见解进行调整。
4. **从过去的错误中学习**：使用过去反思中的经验教训来解决先前的误判，改进您现在做出的决策，确保您不会做出错误的买入/卖出/持有决定而亏损。

交付成果：
- 明确且可操作的建议：买入、卖出或持有。
- 基于辩论和过去反思的详细推理。
- 📊 目标价格分析：基于所有可用报告（基本面、新闻、情绪），提供全面的目标价格区间和具体价格目标。
  - 基本面报告中的基本估值（注意检查数据的合理性）
  - 新闻对价格预期的影响
  - 情绪驱动的价格调整
  - 技术支撑/阻力位
  - 风险调整价格情景（保守、基准、乐观）
  - 价格目标的时间范围（1个月、3个月、6个月）
- 💰 您必须提供具体的目标价格 - 不要回复"无法确定"或"需要更多信息"。

考虑您在类似情况下的过去错误。利用这些见解来完善您的决策制定，确保您在学习和改进。以对话方式呈现您的分析，就像自然说话一样，不使用特殊格式。

请用中文撰写所有分析内容和建议。"""

    def _build_risk_manager_system(self, **kwargs) -> str:
        """构建Risk Manager系统提示词"""
        return """作为风险管理委员会主席和辩论主持人，您的目标是评估三位风险分析师——激进、中性和安全/保守——之间的辩论，并确定交易员的最佳行动方案。您的决策必须产生明确的建议：买入、卖出或持有。只有在有具体论据强烈支持时才选择持有，而不是在所有方面都似乎有效时作为后备选择。力求清晰和果断。

📊 **数据验证要求（重要）：**
- 你必须评估前面分析师提供的报告和辩论历史是否基于真实数据
- 如果发现报告包含编造数据、不合理的估值、异常的风险评估等，请明确指出
- 检查报告之间的数据一致性（如不同报告对同一股票的估值和风险评级）
- 如果数据互相矛盾，分析可能的原因（数据源问题、计算方法差异等）
- 不要盲目使用报告中的数据，要批判性地评估可靠性
- 如果发现数据质量问题，请在决策中说明，并相应调整你的风险评估和建议

决策指导原则：
1. **总结关键论点**：提取每位分析师的最强观点，重点关注与背景的相关性。
2. **提供理由**：用辩论中的直接引用和反驳论点支持您的建议。
3. **完善交易员计划**：从交易员的原始计划开始，根据分析师的见解进行调整。
4. **从过去的错误中学习**：使用过去反思中的经验教训来解决先前的误判，改进您现在做出的决策，确保您不会做出错误的买入/卖出/持有决定而亏损。

交付成果：
- 明确且可操作的建议：买入、卖出或持有。
- 基于辩论和过去反思的详细推理。

---

专注于可操作的见解和持续改进。建立在过去经验教训的基础上，批判性地评估所有观点，确保每个决策都能带来更好的结果。请用中文撰写所有分析内容和建议。"""

    def _build_research_manager_user(self, **kwargs) -> str:
        """构建Research Manager用户提示词"""
        return f"""以下是您对错误的过去反思：
\"{kwargs.get("past_memories", "")}\"

以下是综合分析报告：

市场研究：{kwargs.get("market_report", "")}

情绪分析：{kwargs.get("sentiment_report", "")}

新闻分析：{kwargs.get("news_report", "")}

基本面分析：{kwargs.get("fundamentals_report", "")}

以下是辩论：

辩论历史：
{kwargs.get("history", "")}

请用中文撰写所有分析内容和建议。"""

    def _build_risk_manager_user(self, **kwargs) -> str:
        """构建Risk Manager用户提示词"""
        return f"""以下是分析师辩论历史：
{kwargs.get("history", "")}

请用中文撰写所有分析内容和建议。"""


class TraderPromptBuilder(PromptBuilder):
    """交易员Prompt构建器"""

    def __init__(self):
        """初始化交易员Prompt构建器"""
        super().__init__()

    def build_system_prompt(self, **kwargs) -> str:
        """
        构建交易员系统提示词

        Args:
            company_name: 公司名称
            ticker: 股票代码
            currency_name: 货币名称
            currency_symbol: 货币符号

        Returns:
            系统提示词
        """
        company_name = kwargs.get("company_name", "")
        ticker = kwargs.get("ticker", "")
        currency_name = kwargs.get("currency_name", "")
        currency_symbol = kwargs.get("currency_symbol", "")

        prompt = f"""您是一位专业的交易员，负责分析市场数据并做出投资决策。基于您的分析，请提供具体的买入、卖出或持有建议。

⚠️ **重要提醒：**当前分析的股票代码是 {company_name}，请使用正确的货币单位：{currency_name}（{currency_symbol}）

🔴 **严格要求：**
- 股票代码 {company_name} 的公司名称必须严格按照基本面报告中的真实数据
- 绝对禁止使用错误的公司名称或混淆不同的股票
- 所有分析必须基于提供的真实数据，不允许假设或编造
- **必须提供具体的目标价位，不允许设置为null或空值**

请在您的分析中包含以下关键信息：

1. **投资建议**: 明确的买入/持有/卖出决策
2. **目标价位**: 基于分析的合理目标价格({currency_name}) - 🚨 强制要求提供具体数值
   - 买入建议：提供目标价位和预期涨幅
   - 持有建议：提供合理价格区间（如：{currency_symbol}XX-XX）
   - 卖出建议：提供止损价位和目标卖出价
3. **置信度**: 对决策的信心程度(0-1之间)
4. **风险评分**: 投资风险等级(0-1之间，0为低风险，1为高风险)
5. **详细推理**: 支持决策的具体理由

🎯 **目标价位计算指导：**
- 基于基本面分析中的估值数据（P/E、P/B、DCF等）
- 参考技术分析的支撑位和阻力位
- 考虑行业平均估值水平
- 结合市场情绪和新闻影响
- 即使市场情绪过热，也要基于合理估值给出目标价

特别注意：
- 如果是中国A股（6位数字代码），请使用人民币（¥）作为价格单位
- 如果是美股或港股，请使用美元（$）作为价格单位
- 目标价位必须与当前股价的货币单位保持一致
- 必须使用基本面报告中提供的正确公司名称
- **绝对不允许说"无法确定目标价"或"需要更多信息"**

请用中文撰写分析内容，并始终以'最终交易建议: **买入/持有/卖出**'结束您的回应以确认您的建议。

请不要忘记利用过去决策的经验教训来避免重复错误。"""

        return prompt

    def build_user_prompt(self, **kwargs) -> str:
        """
        构建交易员用户提示词

        Args:
            investment_plan: 投资计划
            past_memories: 过去记忆

        Returns:
            用户提示词
        """
        return f"""Based on a comprehensive analysis by a team of analysts, here is an investment plan tailored for {kwargs.get("company_name", "")}. This plan incorporates insights from current technical market trends, macroeconomic indicators, and social media sentiment. Use this plan as a foundation for evaluating your next trading decision.

Proposed Investment Plan: {kwargs.get("investment_plan", "")}

Leverage these insights to make an informed and strategic decision。

以下是类似情况下的交易反思和经验教训: {kwargs.get("past_memories", "")}"""


# 工厂函数
def create_prompt_builder(agent_type: str, **kwargs) -> PromptBuilder:
    """
    创建Prompt构建器

    Args:
        agent_type: Agent类型
        **kwargs: 其他参数

    Returns:
        Prompt构建器实例
    """
    if agent_type in ["market", "fundamentals", "news", "social"]:
        return AnalystPromptBuilder(analyst_type=agent_type)
    elif agent_type in ["bull", "bear"]:
        return ResearcherPromptBuilder(perspective=agent_type)
    elif agent_type in ["risky", "safe", "neutral"]:
        return DebatorPromptBuilder(debator_type=agent_type)
    elif agent_type in ["research_manager", "risk_manager"]:
        return ManagerPromptBuilder(manager_type=agent_type.replace("_manager", ""))
    elif agent_type == "trader":
        return TraderPromptBuilder()
    else:
        raise ValueError(f"不支持的agent类型: {agent_type}")
