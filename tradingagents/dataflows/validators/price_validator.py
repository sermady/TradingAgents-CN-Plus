...
class PriceValidator(BaseDataValidator):
    ...
    async def cross_validate(
        ...
        result.metadata["sources_failed"] = [
            s for s in sources if s not in sources_with_data
        ]

        return result

    def _validate_current_price(
        self, symbol: str, price: float, result: ValidationResult
    ) -> None:
        """验证当前价格合理性"""
        # 检查价格是否为正数
        if price <= 0:
    ...
    def _validate_bollinger_bands(
        ...
        # 中轨应该在上下轨中间
        if middle is not None and lower is not None and upper is not None:
    ...
    async def _get_data_from_source(
        self, symbol: str, source: str, metric: str
    ) -> Optional[float]:
        """
        ...
        try:
            ...
            if value is not None:
                # 记录成功
                manager.record_source_reliability(
                    source=source, success=True, metric=metric
                )
            else:
                # 记录失败
                manager.record_source_reliability(
                    source=source, success=False, metric=metric, error="指标值为None"
                )
            ...
            try:
                ...
                manager = DataSourceManager()
                manager.record_source_reliability(
                    source=source, success=False, metric=metric, error=str(e)
                )
            except:
                pass
            return None
