...
def create_mongodb_indexes():
    ...
    try:
        ...
        cache_indexes = [
            # 主要键唯一索引
            {'key': [('key', 1)], 'unique': True, 'name': 'cache_key_unique', 'background': True},
            # 创建时间索引
            {'key': [('created_at', -1)], 'name': 'cache_created_at_idx', 'background': True},
            # TTL 索引
            {'key': [('ttl', 1)], 'name': 'cache_ttl_idx', 'background': True},
            # 数据类型 + 创建时间复合索引
            {'key': [('data_type', 1), ('created_at', -1)], 'name': 'cache_type_created_idx', 'background': True},
        ]
        ...
        token_indexes = [
            ...
            # 提供商 + 日期复合索引
            {'key': [('provider', 1), ('date', -1)], 'name': 'provider_date_idx', 'background': True},
        ]
...
def verify_indexes():
    ...
    try:
        ...
        token_usage_collection = db.token_usage
        pipeline = [
            {'$group': {'provider': '$provider', 'date': '$date'}},
             {'$sum': 1, 'count': 1}},
             {'$sort': {'date': -1}},
            {'$limit': 5}
        ]
