...
class TushareRateLimiter(RateLimiter):
    ...
    # Tushare积分等级对应的限流配置 (根据官方文档校准)
    # 120分=free(约50次), 2000分=200次, 5000分=500次
    TIER_LIMITS = {
        "free": {"max_calls": 50, "time_window": 60},  # 免费用户: 实际约50次/分钟
        "basic": {
            "max_calls": 200,
            "time_window": 60,
        },  # 基础用户(2000积分): 200次/分钟
        "standard": {"max_calls": 400, "time_window": 60},  # 标准用户: 400次/分钟
        "premium": {"max_calls": 500, "time_window": 60},  # 5000积分: 500次/分钟
        "vip": {
            "max_calls": 500,
            "time_window": 60,
        },  # VIP用户(5000积分): 修正为500次/分钟
        "svip": {
            "max_calls": 1000,
            "time_window": 60,
        },  # 超级VIP(10000积分): 1000次/分钟
    }
